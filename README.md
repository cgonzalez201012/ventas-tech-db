...
* `ventas_tech_db.sql` → Checkpoint Módulo 3. Crea la base `Ventas_Tech_DB` con 4 tablas (`categorias`, `clientes`, `productos`, `ventas`), sus relaciones (Foreign Keys) y carga los datos de prueba iniciales. Más adelante, para el Módulo 5, le agregué dos registros más: un cliente sin ventas y un producto sin ventas, para poder probar las consultas con LEFT JOIN.
...
* `m5_consultas_joins.sql` → Pre-entrega Módulo 5. Son consultas que cruzan las 4 tablas con JOINs: una vista base con INNER JOIN (ventas + clientes + productos + categorías), dos consultas con LEFT JOIN para encontrar clientes y productos sin movimiento, y un consolidado por período usando UNION ALL. Esta vista es la que voy a usar más adelante como base del dashboard en Power BI.
...
4. Para las consultas con JOINs del Módulo 5, abrir `m5_consultas_joins.sql` y ejecutarlo también sobre `Ventas_Tech_DB`.

Todos los scripts están comentados por secciones para que se entienda qué hace cada parte.
